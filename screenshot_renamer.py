...
parser.add_argument("-t", dest="ticket_num", help="Ticket number")
...
def convert_time(base_file_path, file_name):
    my_time_stamp = strftime("%Y%m%d", strptime(time.ctime(os.path.getmtime('{}/{}'.format(base_file_path,file_name)))))
    return my_time_stamp

def new_namer(base_file_path,before_file_name,ss_date,ticket_num,ss_desc):
    if ticket_num != "":
       my_ticket_num = '{}_'.format(ticket_num)
    else:
        my_ticket_num = ""
    my_before_file = '{}/{}'.format(base_file_path,before_file_name)
    file_ext = before_file_name[-4:]
    my_after_file = '{}/{}{}_{}{}'.format(base_file_path,my_ticket_num,ss_date,ss_desc,file_ext)
...
def rename_ui(files_to_rename,base_file_path,ticket_num,ss_date):
    for current_file in files_to_rename:
        ...
        if 's' in user_choice:
            ...
            my_after_file = new_namer(base_file_path,current_file,ss_date,ticket_num,ss_desc)
...
def rename_ui_onefile(base_file_path,file_to_modify,ticket_num,ss_date):
    ...
    if 's' in user_choice:
       ...
       my_after_file = new_namer(base_file_path,file_to_modify,ss_date,ticket_num,ss_desc)
       print("Rename to {}?".format(my_after_file))
       print("Enter f to confirm or c to cancel")
       user_choice = get_user_input()
       if 'c' in user_choice:
...
def initialize_values():
    ...
    ticket_num = input('Enter a JIRA ticket number: ')
    return ss_date, ticket_num
...
if args.file_to_modify is not None:
    ...
    if args.ticket_num is not None:
        ticket_num = args.ticket_num
    else:
        ticket_num = ""
    rename_ui_onefile(dir_path,file_to_modify,ticket_num,ss_date)
else:
    user_values = initialize_values()
    ss_date = user_values[0]
    ticket_num = user_values[1]
    ...
    rename_ui(today_files,picsdir,ticket_num,ss_date)
